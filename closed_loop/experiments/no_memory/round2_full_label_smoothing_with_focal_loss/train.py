...
parser.add_argument('--label_smoothing', default=0.1, type=float, help='label smoothing epsilon')
parser.add_argument('--focal_gamma', default=2.0, type=float, help='focal loss gamma parameter')
parser.add_argument('--focal_alpha', default=None, type=float, help='focal loss alpha parameter (optional)')
...
# Define Label Smoothing with Focal Loss
class LabelSmoothingFocalLoss(nn.Module):
    def __init__(self, num_classes=10, smoothing=0.1, gamma=2.0, alpha=None, reduction='mean'):
        ...
        if self.alpha is not None:
            if isinstance(self.alpha, (list, tuple)):
                ...
                self.alpha = torch.full((num_classes,), self.alpha)
            else:
                raise TypeError('alpha must be float, list or tuple')
    ...
    def forward(self, pred, target):
        ...
        ce_loss = -torch.sum(smooth_target * log_pred, dim=-1)
        
        # Calculate focal weight
        pred_prob = torch.exp(log_pred)
        pt = pred_prob.gather(1, target.unsqueeze(1)).squeeze()
