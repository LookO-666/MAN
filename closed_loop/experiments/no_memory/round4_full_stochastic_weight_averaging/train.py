...
parser.add_argument('--swa_start', default=160, type=int, help='epoch to start SWA')
parser.add_argument('--swa_freq', default=1, type=int, help='SWA model sampling frequency')
...
swa_model.load_state_dict(net.state_dict())
swa_n = 0
...
def test(epoch, model=None):
    ...
    if model is net and acc > best_acc:
        print('Saving..')
        state = {
            'net': net.state_dict(),
            'acc': acc,
...
for epoch in range(start_epoch, start_epoch + args.epoch):
    ...
    # Update SWA model
    if epoch >= args.swa_start and (epoch - args.swa_start) % args.swa_freq == 0:
...
if swa_n > 0:
    ...
    torch.optim.swa_utils.update_bn(trainloader, swa_model, device=device)
    swa_loss, swa_acc = test(epoch, model=swa_model)
    results.append({
        'epoch': epoch + 1,
        'train_loss': None,
        'train_acc': None,
...
best_test_acc = max(r['test_acc'] for r in results if r['test_acc'] is not None)
output = {
